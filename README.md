# Go API client for ResolutionAPI
...
```golang
import ResolutionAPI "github.com/superlink-me/resolution-api-go-client"
```
...
```golang
ctx := context.WithValue(context.Background(), ResolutionAPI.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), ResolutionAPI.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), ResolutionAPI.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), ResolutionAPI.ContextOperationServerVariables, map[string]map[string]string{
