# Go API client for SuperlinkAPI
...
```golang
import SuperlinkAPI "github.com/superlink-me/superlink-api-go-client"
```
...
```golang
ctx := context.WithValue(context.Background(), SuperlinkAPI.ContextServerIndex, 1)
```
...
```golang
ctx := context.WithValue(context.Background(), SuperlinkAPI.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
```golang
ctx := context.WithValue(context.Background(), SuperlinkAPI.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), SuperlinkAPI.ContextOperationServerVariables, map[string]map[string]string{
