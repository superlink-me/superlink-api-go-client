...
type ApiSubdomainInvalidateClaimRateLimitRequest struct {
	ctx context.Context
	ApiService *SubdomainAPIService
	parentDomain string
}

func (r ApiSubdomainInvalidateClaimRateLimitRequest) Execute() (*http.Response, error) {
	return r.ApiService.SubdomainInvalidateClaimRateLimitExecute(r)
}

/*
SubdomainInvalidateClaimRateLimit Invalidates the claim rate limit for the current IP

Invalidates the claim rate limit for the current IP

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param parentDomain superlink.me
 @return ApiSubdomainInvalidateClaimRateLimitRequest
*/
func (a *SubdomainAPIService) SubdomainInvalidateClaimRateLimit(ctx context.Context, parentDomain string) ApiSubdomainInvalidateClaimRateLimitRequest {
	return ApiSubdomainInvalidateClaimRateLimitRequest{
		ApiService: a,
		ctx: ctx,
		parentDomain: parentDomain,
...
// Execute executes the request
func (a *SubdomainAPIService) SubdomainInvalidateClaimRateLimitExecute(r ApiSubdomainInvalidateClaimRateLimitRequest) (*http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "SubdomainAPIService.SubdomainInvalidateClaimRateLimit")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/parentdomains/{parentDomain}/invalidate-claim-rate-limit"
	localVarPath = strings.Replace(localVarPath, "{"+"parentDomain"+"}", url.PathEscape(parameterValueToString(r.parentDomain, "parentDomain")), -1)
